...
export function CicdImageBrowserPage() {
  ...
  useEffect(() => {
    ...
    setArtifacts([]);
    void loadProjects();
  }, [registryId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const columns: ColumnsType<HarborTagItem> = [
    {
      title: "Tags",
      dataIndex: "tags",
      render: (tags: string[]) =>
    ...
    { title: "Digest", dataIndex: "digest", ellipsis: true, width: 280 },
    {
      title: "大小",
      dataIndex: "size",
      width: 100,
      render: (n?: number) => (n ? `${(n / 1024 / 1024).toFixed(1)} MB` : "—"),
    },
    {
      title: "推送时间",
      dataIndex: "push_time",
    ...
    },
    {
      ...
      width: 180,
      render: (links?: HarborTagItem["linked_build_runs"]) =>
        links?.length ? (
          <Space wrap>
            {links.map((l) => (
  ...
  return (
    <div className="page-stack">
      <PageTelemetryHeader
        ...
        meta={[artifacts.length ? `TAGS / ${artifacts.length}` : ""]}
      />
      <Card bordered={false}>
        <Space wrap style={{ marginBottom: 16 }}>
          ...
          <Select
            ...
            showSearch
          />
          <Button icon={<ReloadOutlined />} onClick={() => void (repository ? loadArtifacts() : loadRepos())}>
            刷新
          </Button>
        </Space>
        {!registryId ? (
          ...
          <Table rowKey={(r) => r.digest || r.tags?.join(",") || Math.random().toString()} loading={loading} columns={columns} dataSource={artifacts} pagination={{ pageSize: 20 }} />
        )}
      </Card>
    </div>
  );
