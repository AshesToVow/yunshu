...
<Space direction="vertical" style={{ width: "100%" }} size="middle">
                ...
                <Alert
                  ...
                  showIcon
                  message="云到期规则说明"
                  description="后台在「启用定时自动评估」且已填写 Cron 时拉取云实例到期时间：① 告警服务以固定节拍（约每 5 秒）检查是否到达各规则在控制台配置的 Cron；② 与 YAML 中 alert.monitor_eval_cron_spec（仅用于内置 PromQL 监控规则）无关；③ 仅当剩余天数 ≤ 提前天数才 ctx.firing。须配置 security.encryption_key 且云账号可解密。订阅按 labels 匹配，route 等不会自动替代订阅节点。"
                />
                <Table rowKey="id" columns={ctx.cloudExpiryColumns} dataSource={ctx.cloudExpiryList} pagination={false} scroll={{ x: 1280 }} />
              </Space>
  );
}
