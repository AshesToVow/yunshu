...
| 阶段           | 条件               | 说明                                                                           |
| ------------ | ---------------- | ---------------------------------------------------------------------------- |
| CheckOut     | publishMode ≠ 回滚 | `pipeline.checkoutCode()`，记录 `GIT_COMMIT`                                    |
| 代码编译         | 同上               | `compileFrontend()` + 依赖选项；制品名 `{JOB_NAME}-{BUILD_TIME}-{GIT_COMMIT}.tar.gz` |
| 打包并上传至 MinIO | 同上               | `tar -czf dist/` + `uploadToMinio()` + `pruneMinioArtifacts()`               |
| 发布           | auto / manual    | `runPublish()` → `deployFrontend()` + chown                                  |
| 回滚           | rollback         | `selectRollbackVersion()` → `deployFrontend()` + chown                       |
