...
func (s *MenuService) normalizeMenuHierarchyAndSort(ctx context.Context) (bool, error) {
	all, err := s.menuRepo.ListAll(ctx)
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "normalizeMenuHierarchyAndSort", err)
	}
	changed := false
...
func (s *MenuService) ensureHiddenMenusDisabled(ctx context.Context) (bool, error) {
	all, err := s.menuRepo.ListAll(ctx)
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "ensureHiddenMenusDisabled", err)
	}
	changed := false
	for i := range all {
		m := &all[i]
...
func (s *MenuService) repairEventMenus(ctx context.Context) (bool, error) {
	all, err := s.menuRepo.ListAll(ctx)
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "repairEventMenus", err)
	}
	changed := false
	for i := range all {
		m := &all[i]
...
func (s *MenuService) ensureBuiltInMenus(ctx context.Context, tree []model.Menu) (bool, error) {
	...
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "ensureBuiltInMenus", err)
	}
	if k8sChanged {
		changed = true
	}
...
func (s *MenuService) ensureOrgMenus(ctx context.Context) (bool, error) {
	all, err := s.menuRepo.ListAll(ctx)
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "ensureOrgMenus", err)
	}
	required := []model.Menu{
...
func (s *MenuService) ensureSystemMenus(ctx context.Context) (bool, error) {
	all, err := s.menuRepo.ListAll(ctx)
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "ensureSystemMenus", err)
	}
	changed := false
...
func (s *MenuService) ensureProjectMgmtMenus(ctx context.Context) (bool, error) {
	all, err := s.menuRepo.ListAll(ctx)
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "ensureProjectMgmtMenus", err)
	}
...
func (s *MenuService) ensureK8sMenus(ctx context.Context, tree []model.Menu) (bool, error) {
	...
	for _, r := range required {
		...
		m.ParentID = &parentID
		if err := s.menuRepo.Create(ctx, &m); err != nil {
			return false, bizerrors.Pass(ctx, "menu", "ensureK8sMenus", err)
		}
		changed = true
	}
	return changed, nil
...
func (s *MenuService) ensureAlertNotifyMenus(ctx context.Context) (bool, error) {
	all, err := s.menuRepo.ListAll(ctx)
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "ensureAlertNotifyMenus", err)
	}
...
func (s *MenuService) hideRuntimeConfigMenu(ctx context.Context) (bool, error) {
	all, err := s.menuRepo.ListAll(ctx)
	if err != nil {
		return false, bizerrors.Pass(ctx, "menu", "hideRuntimeConfigMenu", err)
	}
	changed := false
	for i := range all {
		m := &all[i]
