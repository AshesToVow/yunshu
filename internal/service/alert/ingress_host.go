...
type IngressHost interface {
	...
	LogSilenceSuppressed(ctx context.Context, title string, severity string, status string, envLabel string, groupKey string, labelsDigest string, silenceID uint, payload map[string]any)
	...
	LogInhibitionEvent(ctx context.Context, title string, severity string, status string, envLabel string, groupKey string, labelsDigest string, inhEvent *model.AlertInhibitionEvent, outgoing map[string]any)
	...
	EnrichOutgoingProjectName(ctx context.Context, outgoing map[string]any)
	EnrichAssigneeAndDutyEmails(ctx context.Context, outgoing map[string]any, labels map[string]string)
	...
	LogSuppressedFiringTiming(ctx context.Context, title, severity, status, groupKey, labelsDigest, reason string, outgoing map[string]any)
	ChannelRouteForAlert(ctx context.Context, status string, labels map[string]string, fingerprint string) ChannelRoute
	ExpandChannelSetForAssigneeNotification(ctx context.Context, channels map[uint]struct{}, receiverGroupIDs []uint, outgoing map[string]any)
	...
	LogNoMatchedChannel(ctx context.Context, title, severity, status, envLabel, groupKey, labelsDigest string, outgoing map[string]any, reason string)
	...
	LogSuppressedRouteSilence(ctx context.Context, title, severity, status, envLabel, groupKey, labelsDigest string, silenceSeconds int, outgoing map[string]any)
	WasFiringDelivered(ctx context.Context, fingerprint string) bool
	LogResolvedSuppressedNoPriorFiringDelivery(ctx context.Context, title, severity, status, groupKey, labelsDigest string, outgoing map[string]any)
	...
	LogSuppressedResolvedAggregate(ctx context.Context, title, severity, status, groupKey string, outgoing map[string]any)
	ChannelMatchesAlert(channel model.AlertChannel, labels map[string]string) bool
	SendToChannel(ctx context.Context, channel *model.AlertChannel, source, title, severity, status string, outgoing map[string]any) (int, error)
	...
	LogAllChannelsDeliveryFailed(ctx context.Context, title, severity, status, envLabel, groupKey, labelsDigest string, outgoing map[string]any)
