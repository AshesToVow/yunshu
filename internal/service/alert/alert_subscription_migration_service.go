...
func (s *AlertSubscriptionService) MigrateFromPolicies(ctx context.Context, opts MigrateFromPoliciesOptions) (*SubscriptionMigrationReport, error) {
	...
	err := s.repo.WithTx(ctx, func(tx *gorm.DB) error {
		return s.migrateFromPoliciesTx(ctx, tx, disableOld, fallbackProject, rep)
	})
	if err != nil {
		return nil, err
	}
...
func (s *AlertSubscriptionService) migrateFromPoliciesTx(ctx context.Context, tx *gorm.DB, disableOld bool, fallbackProject uint, rep *SubscriptionMigrationReport) error {
	...
	if err := tx.WithContext(ctx).Table("alert_policies").Find(&policies).Error; err != nil {
	...
	getOrCreateRoot := func(projectID uint) (*model.AlertSubscriptionNode, error) {
		...
		if err2 := tx.WithContext(ctx).Create(&root).Error; err2 != nil {
			return nil, err2
		}
		rootByProject[projectID] = &root
		return &root, nil
	...
	for i := range policies {
		...
		if disableOld && p.Enabled {
			_ = tx.WithContext(ctx).Table("alert_policies").Where("id = ?", p.ID).Update("enabled", false).Error
...
func (s *AlertSubscriptionService) firstEnabledProjectID(ctx context.Context) (uint, error) {
	var p model.Project
	err := s.repo.WithTx(ctx, func(tx *gorm.DB) error {
		return tx.WithContext(ctx).Where("status = ?", 1).Order("id ASC").First(&p).Error
	})
	if err != nil {
