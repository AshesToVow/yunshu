...
// ServerDetailItem ? ServerItem ????? SSH ??????????????
...
func (s *Service) RunCloudServerAction(ctx context.Context, projectID, serverID uint, req CloudServerActionRequest) (*CloudServerActionResult, error) {
	...
	case "reset_password":
		...
		return &CloudServerActionResult{ServerID: serverID, Action: action, Message: "?????????"}, nil
	case "reboot":
...
func (s *Service) ServersImportTemplateExcel() (*excelize.File, error) {
	...
	notes := [][]any{
		{"??", "??", "??", "??/??"},
		{"name", "?????", "?", "app-01"},
		{"host", "????/IP", "?", "10.0.0.12 / example.com"},
		{"port", "SSH ????? 22?", "?", "22"},
		{"os_type", "????????? linux?", "?", "linux / windows / darwin"},
		{"tags", "????????", "?", "prod,web"},
		{"auth_type", "??????? password?", "?", "password / key"},
		{"username", "SSH ???", "?", "root"},
		{"password", "SSH ???auth_type=password ????", "?", "your_password"},
		{"private_key", "SSH ???auth_type=key ????PEM ???", "?", "-----BEGIN ..."},
		{"passphrase", "????????", "?", ""},
		{"??", "?????????????????????????????????????", "", ""},
	}
