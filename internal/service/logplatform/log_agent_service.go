...
func randomToken() (string, error) {
	...
	if _, err := rand.Read(buf); err != nil {
		return "", bizerrors.Pass(context.Background(), "log-agent", "randomToken", err)
	}
...
func (s *LogAgentService) AllowedLogSourceIDs(ctx context.Context, agent *model.LogAgent) (map[uint]struct{}, error) {
	...
	if err != nil {
		return nil, bizerrors.Pass(ctx, "log-agent", "AllowedLogSourceIDs", err)
	}
...
func (s *LogAgentService) ReportHealthByToken(ctx context.Context, req LogAgentHealthReportRequest) error {
	...
	if err != nil {
		return bizerrors.Pass(ctx, "log-agent", "ReportHealthByToken", err)
	}
	now := time.Now()
...
func (s *LogAgentService) RecordOfflineEpisodes(ctx context.Context) error {
	...
	if err != nil {
		return bizerrors.Pass(ctx, "log-agent", "RecordOfflineEpisodes", err)
	}
	now := time.Now()
	for i := range agents {
		ag := &agents[i]
...
func (s *LogAgentService) ListByProject(ctx context.Context, q LogAgentListQuery) ([]LogAgentListItem, error) {
	...
	if err != nil {
		return nil, bizerrors.Pass(ctx, "log-agent", "ListByProject", err)
	}
...
func (s *LogAgentService) buildListItem(ctx context.Context, projectID uint, sv model.Server) (*LogAgentListItem, error) {
	...
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, bizerrors.Pass(ctx, "log-agent", "buildListItem", err)
	}
...
func (s *LogAgentService) BatchRefreshHeartbeat(ctx context.Context, req AgentBatchHeartbeatRefreshRequest) (*AgentBatchHeartbeatRefreshResult, error) {
	...
	if err != nil {
		return nil, bizerrors.Pass(ctx, "log-agent", "BatchRefreshHeartbeat", err)
	}
...
func (s *LogAgentService) Status(ctx context.Context, projectID, serverID, logSourceID uint) (*LogAgentStatusResult, error) {
	...
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, bizerrors.Pass(ctx, "log-agent", "Status", err)
	}
	reported := ""
	if err == nil && agent != nil {
		out.Name = agent.Name
...
func (s *LogAgentService) RuntimeConfigByToken(ctx context.Context, token string) (*AgentRuntimeConfigResult, error) {
	...
	if err != nil {
		return nil, bizerrors.Pass(ctx, "log-agent", "RuntimeConfigByToken", err)
	}
...
func (s *LogAgentService) DeleteForProject(ctx context.Context, projectID, agentID uint) error {
	if _, err := s.repo.GetByIDAndProject(ctx, agentID, projectID); err != nil {
		...
		return bizerrors.Pass(ctx, "log-agent", "DeleteForProject", err)
	}
...
// MaxLogHistoryPerStream 单日志流内存缓冲上限（与 Agent 推送、历史查询一致）。
