...
%s      topic: %s
      codec:
        type: json%s
`, brokers.String(), quoteYAML(kafkaCfg.Topic), sasl)
	}
...
func renderPipelinesYAML(projectID, serverID uint, entries []LoggiePipelineSourceEntry, esCfg config.ElasticsearchConfig, kafkaCfg config.KafkaConfig) string {
	...
	if kafkaCfg.SinkViaKafka() {
		body.WriteString(fmt.Sprintf("# project_id=%d server_id=%d pipelines=%d sink=kafka topic=%s\n", projectID, serverID, len(entries), kafkaCfg.Topic))
	} else {
