...
func renderPipelineEntry(projectID, serverID uint, entry LoggiePipelineSourceEntry, sinkBlock string) string {
...
// renderSinkYAML 按开关生成 ES 直写或 Kafka 中转 sink。
func renderSinkYAML(serverID uint, serverHost string, esCfg config.ElasticsearchConfig, kafkaCfg config.KafkaConfig) string {
	...
	if kafkaCfg.SinkViaKafka() {
      ...
      sasl:
		...
		topic := AgentKafkaTopicTemplate(host, kafkaCfg.TopicPrefix)
...
func renderPipelinesYAML(projectID, serverID uint, entries []LoggiePipelineSourceEntry, esCfg config.ElasticsearchConfig, kafkaCfg config.KafkaConfig) string {
	esCfg = esCfg.Normalized()
	kafkaCfg = kafkaCfg.Normalized()
	serverHost := ""
	...
	if serverHost == "" {
		serverHost = fmt.Sprintf("server-%d", serverID)
	}
	sinkBlock := renderSinkYAML(serverID, serverHost, esCfg, kafkaCfg)

	var body strings.Builder
	body.WriteString("# Generated by Yunshu Loggie Bootstrap\n")
	if kafkaCfg.SinkViaKafka() {
		body.WriteString(fmt.Sprintf("# project_id=%d server_id=%d pipelines=%d sink=kafka topic=%s group=%s\n",
			projectID, serverID, len(entries), AgentKafkaTopicTemplate(serverHost, kafkaCfg.TopicPrefix), kafkaCfg.ConsumerGroup))
	} else {
		body.WriteString(fmt.Sprintf("# project_id=%d server_id=%d pipelines=%d index=%s\n", projectID, serverID, len(entries), AgentIndexPattern(serverHost)))
	}
	body.WriteString("pipelines:\n")
	for _, e := range entries {
		body.WriteString(renderPipelineEntry(projectID, serverID, e, sinkBlock))
	}
	return body.String()
}

func renderSystemConfigYAML(projectID, serverID uint, monitorPort int) string {
	return fmt.Sprintf(`# Generated by Yunshu — system config only (-config.system)
# project_id=%d server_id=%d
# 成对使用：本文件 + pipelines.yml（不要用本文件兼做采集管道）
loggie:
  reload:
    enabled: true
    period: 10s
  ...
  monitor:
    ...
    listeners:
      ...
      sink: ~
      queue: ~
`, projectID, serverID, defaultMonitorPort(monitorPort))
}

func renderFullPipelineYAML(projectID, serverID uint, monitorPort int, entries []LoggiePipelineSourceEntry, esCfg config.ElasticsearchConfig, kafkaCfg config.KafkaConfig) string {
	// 兼容旧下载包：完整文件 = 系统配置 + pipelines（仅供预览；线上用分体文件）
	sys := renderSystemConfigYAML(projectID, serverID, monitorPort)
	pipelines := renderPipelinesYAML(projectID, serverID, entries, esCfg, kafkaCfg)
	return sys + "\n" + strings.TrimPrefix(pipelines, "# Generated by Yunshu Loggie Bootstrap\n")
}
...
func BuildMultiPipelineBundle(
	...
	if len(entries) == 0 {
		...
		return BuildPipelineBundle(opts, esCfg, kafkaCfg, token, yunshuBaseURL)
	}
	...
	pipelinesOnly := renderPipelinesYAML(projectID, serverID, entries, esCfg, kafkaCfg)
	systemYAML := renderSystemConfigYAML(projectID, serverID, monitorPort)
