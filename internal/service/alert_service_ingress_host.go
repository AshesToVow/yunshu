...
func (s *AlertService) ingressHost() alertsvc.IngressHost {
...
func (h alertIngressHost) ChannelRouteForAlert(ctx context.Context, status string, labels map[string]string) alertsvc.ChannelRoute {
	r := h.s.channelRouteForAlert(ctx, status, labels)
	return alertsvc.ChannelRoute{
...
func canonicalItemsToIngress(items []CanonicalIngressAlert) []alertsvc.IngressCanonicalItem {
	out := make([]alertsvc.IngressCanonicalItem, len(items))
	for i, ca := range items {
		a := ca.Alert
		out[i] = alertsvc.IngressCanonicalItem{
			Source:            ca.Source,
			...
			Alert: alertsvc.IngressAlertDetail{
