...
func applyAlertEventProjectFilter(tx *gorm.DB, db *gorm.DB, projectID uint) *gorm.DB {
	...
	// 优先 project_id 列（已回填）；仅历史未解析行才走 datasource / 策略关联，避免 CSV 匹配扫全表。
	contains := database.SQLCSVContainsID(database.DialectName(db), "n.id", "alert_events.matched_policy_ids")
	return tx.Where(
		fmt.Sprintf(`project_id = ?
OR (COALESCE(project_id, 0) = 0 AND datasource_id IN (?))
OR (COALESCE(project_id, 0) = 0 AND matched_policy_ids <> '' AND EXISTS (
