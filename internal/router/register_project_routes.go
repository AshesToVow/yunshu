...
func RegisterProjectRoutes(api *gin.RouterGroup, d *RouteDeps) {
	...
	projectScoped.GET("/change-events/conflict-check", d.projectCatalogHandler.ConflictCheck)
	projectScoped.GET("/change-freeze-windows", d.projectCatalogHandler.ListFreezeWindows)
	projectScoped.POST("/change-freeze-windows", d.projectCatalogHandler.UpsertFreezeWindow)
	projectScoped.DELETE("/change-freeze-windows/:freezeId", d.projectCatalogHandler.DeleteFreezeWindow)
	projectScoped.GET("/incident-context", d.projectCatalogHandler.IncidentContext)
	projectScoped.GET("/incidents", d.projectCatalogHandler.ListIncidents)
	projectScoped.POST("/incidents", d.projectCatalogHandler.OpenIncident)
	projectScoped.PUT("/incidents/:incidentId", d.projectCatalogHandler.UpdateIncident)
	projectScoped.GET("/incidents/:incidentId/timeline", d.projectCatalogHandler.GetIncidentTimeline)
	projectScoped.POST("/incidents/:incidentId/notes", d.projectCatalogHandler.AddIncidentNote)
}
