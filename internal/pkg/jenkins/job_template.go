...
func BuildPipelineJobConfigXML(in JobTemplateInput) string {
	...
	if k8sPipeline {
		...
		params.WriteString(stringParam("deployStrategy", "rolling", "rolling|canary|blue_green"))
		params.WriteString(stringParam("canaryReplicas", "1", "金丝雀初始副本"))
		params.WriteString(stringParam("canaryPercent", "10", "金丝雀流量占比提示"))
		params.WriteString(stringParam("canarySteps", "10,50,100", "金丝雀晋级步骤"))
		params.WriteString(stringParam("blueGreenService", "", "蓝绿 Service 名（空则 {imageName}-service）"))
