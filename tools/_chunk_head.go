...
// rechunkDocument 鎸?Markdown 鏍囬鍒囩墖銆?func (s *Service) rechunkDocument(ctx context.Context, doc *model.AiKbDocument) error {
...
func splitMarkdownChunks(content string) []mdChunk {
	...
	// 鍚堝苟杩囩煭鍧?	merged := make([]mdChunk, 0, len(out))
	for _, c := range out {
